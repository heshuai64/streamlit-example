...
if st.session_state.data_check:
    with st.expander("供应链各项成本结果：", expanded=True):
        st.markdown(f"***权重系数:***")
        col_w1, col_w2, col_w3, col_w4 = st.columns([1,1,1,0.8], gap='large')
        with col_w1:
    ...
    if st.session_state.data_check:
        with st.expander("遍历计算各海空比的供应链成本：", expanded=True):
            ...
            with col81:
                min_ocean_rate = st.number_input("海空比最小值:", value=0.01, min_value=0.0099, max_value=0.99,step=0.01)
            with col82:
                max__ocean_rate = st.number_input("海空比最大值:", value=0.99, min_value=0.01, max_value=0.99,step=0.01)
            with col83:
                sep_ocean_rate = st.number_input("步长:", value=0.05, min_value=0.01, max_value=1.00,step=0.01)
